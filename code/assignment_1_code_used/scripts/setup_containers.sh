...
# docker exec -it mysimbdp /bin/bash -c 'apt install python3.11-venv && python3 -m venv .venv && source .venv/bin/activate && python3 -m pip install pyspark[sql] && python3 -m pip install kafka-python'
