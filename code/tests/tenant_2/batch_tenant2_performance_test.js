...
async function runPerformanceTest() {
    ...
    await saveSourceDataSize(sourceData.slice((0.6 * sourceData.length), (sourceData.length)))
    ...
    for (let i = 6; i < 10; i++) {
        ...
        for (let j = startIndex; j < endIndex; j++) {
            await streamInput.putFilesIntoStreamInput(testData[j], '2');
        }
...
async function saveIterationTimes(iterationTimes) {
    const relativeFilePath = path.join(__dirname, 'batch_tenant_2_performance_test.json');
    await fs.writeFile(relativeFilePath, JSON.stringify(iterationTimes));
}

async function saveFullIterationTime(iterationTimes) {
    const relativeFilePath = path.join(__dirname, 'batch_tenant_2_full_performance_time.json');
    await fs.writeFile(relativeFilePath, JSON.stringify(iterationTimes));
}

async function saveSourceDataSize(sourceData) {
    const relativeFilePath = path.join(__dirname, 'batch_tenant_2_performance_source_data_size.json');
